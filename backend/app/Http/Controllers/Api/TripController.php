...
class TripController extends Controller
{
    public function upcoming(): JsonResponse
    {
        ...
        $slots = $trips
            ->groupBy(fn ($trip) => $trip->trip_date->toDateString().'|'.$trip->departure_time.'|'.$trip->direction)
            ->map(function ($slotTrips): array {
                $first = $slotTrips->first();
                $totalSeats = $slotTrips->sum(fn ($t) => $t->bus?->seat_count ?? 0);
                ...
                return [
                    ...
                    'confirmation_deadline' => $slotTrips
                        ->filter(fn ($t) => $t->confirmation_deadline)
